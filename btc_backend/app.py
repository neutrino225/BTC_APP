...
def login():
    if request.method == 'POST':
        ...
        with sqlite3.connect('users.db') as conn:
            cursor = conn.cursor()
            cursor.execute('SELECT account_number FROM customer_info WHERE username = ? AND password = ?', (username, password))
            ...
            if user:
                ...
                # Return the session_id and account_number to the client
                return jsonify({'message': 'Login successful', 'sender_id': session_id, 'account_number': user[0]}), 200
            else:
...
#     with sqlite3.connect('users.db') as conn:
#         cursor = conn.cursor()
#         cursor.execute('SELECT account_number FROM customer_info WHERE username = ? AND password = ?',
#                        (username, password))
#         user = cursor.fetchone()
...
def dashboard():
    ...
    username = session.get('username', 'User')  # Default to 'User' if username is not found
...
def fetch_balance_from_db(account_number):
    with sqlite3.connect('users.db') as conn:
        cursor = conn.cursor()
        cursor.execute('SELECT outstanding_balance FROM customer_info WHERE account_number = ?', (account_number,))
...
def get_user_data():
    ...
    allowed_columns = ['username', 'cnic', 'account_number', 'account_title',
                       'customer_name', 'contact_number', 'email', 'ntn', 'cif', 'account_type_name',
                       'account_type_code', 'outstanding_balance', 'date_of_birth', 'address',
                       'nationality', 'city_residential', 'registration_number', 'father_name',
                       'reward_points']
...
def query_user_info(account_number, fields):
    with sqlite3.connect('users.db') as conn:
        ...
        query = f"SELECT {', '.join(fields)} FROM customer_info WHERE account_number = ?"
        cursor.execute(query, (account_number,))
        result = cursor.fetchone()
...
def add_payee():
    ...
    if payee:
        ...
        if bank_name.lower() == "allied bank":
            payee_name = cursor.execute("SELECT account_title FROM customer_info WHERE account_number=?", (payee_account_number,)).fetchone()
            payee_name = payee_name[0] if payee_name else None
            # Insert new payee
            if payee_name:
                cursor.execute("INSERT INTO payees_info (account_number, payee_name, payee_account_number, iban, bank_name, nickname) VALUES (?, ?, ?, ?, ?, ?)",
                               (account_number, payee_name, payee_account_number, iban, bank_name, nickname))
                conn.commit()
                conn.close()
                return jsonify({'status': 'added', 'message': f"Payee {payee_name} has been successfully added"}), 201
...
def remove_payee():
    ...
    if payee:
        ...
        conn.commit()
        conn.close()
        return jsonify({'status': 'removed', 'message': f"Payee {payee[1]} has been successfully added"}), 200
    else:
        conn.close()
        return jsonify({'status': 'not found', 'message': 'Payee not found.'}), 404
...
def get_transactions(account_number, limit=None, start_date=None, end_date=None, category=None):
    ...
    transactions = [
                tx for tx in transactions
                if (not start_date or datetime.strptime(tx[5], '%d/%m/%Y %H:%M:%S') >= datetime.strptime(start_date_formatted, '%d/%m/%Y %H:%M:%S')) and
                   (not end_date or datetime.strptime(tx[5], '%d/%m/%Y %H:%M:%S') <= datetime.strptime(end_date_formatted, '%d/%m/%Y %H:%M:%S'))
            ]
...
def check_payee_info():
    ...
    if payee:
        ...
        return jsonify({"status": "not_found", "message": "No payee found with this account number or nickname."})
...
def get_customer_info():
    ...
    conn = connect_db()
    cursor = conn.cursor()
    cursor.execute("SELECT customer_name FROM customer_info WHERE account_number = ?", (account_number,))
...
def get_centralized_data():
    ...
    conn = connect_db()
    cursor = conn.cursor()
    cursor.execute("SELECT name FROM centralized_banking WHERE account_number = ?", (account_number,))
...
def transfer_funds():
    ...
    cursor.execute("SELECT outstanding_balance FROM customer_info WHERE account_number = ?", (from_account,))
    ...
    cursor.execute("UPDATE customer_info SET outstanding_balance = outstanding_balance - ? WHERE account_number = ?", (amount, from_account))
    cursor.execute("UPDATE customer_info SET outstanding_balance = outstanding_balance + ? WHERE account_number = ?", (amount, to_account))
    ...
    reference_id = str(random.randint(1000000000, 9999999999))  # Generate random 10-digit reference ID
    ...
    cursor.execute(
        """INSERT INTO transactions_info (sr, reference_id, `from`, `to`, amount, type, dateTime, category, description)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)""",
        (new_sr, reference_id, from_account, to_account, -amount, transaction_type, dateTime, category, description)
    )
    conn.commit()
    conn.close()

    return jsonify({"status": "success", "message": f"Transfer of {amount} completed successfully."})
...
def check_complaints():
    ...
    if not account_number:
        return jsonify({"status": "failure", "message": "Account number is required."}), 400
    ...
    try:
        ...
        if not complaints:
            return jsonify({"status": "success", "message": "No complaints found for this account."})
...
def register_complaint():
    ...
    if not account_number or not description:
        return jsonify({"status": "failure", "message": "Account number and description are required."}), 400
    ...
    try:
        ...
        print(f"Inserting Complaint: ID: {complaint_id}, Account: {account_number}, Description: {description}, Status: {status}")
...
def add_user_bill():
    ...
    if not all([account_number, bill_id, nickname]):
        return jsonify({"status": "failure", "message": "Account Number, Bill ID, and Nickname are required."}), 400
    ...
    try:
        ...
        if existing_bill_count > 0:
            ...
            return jsonify({"status": "failure",
                            "message": f"A bill with the nickname '{nickname}' is already added. Please use a different nickname."}), 400
        ...
        return jsonify({"status": "success", "message": "User bill added successfully."})
...
def pay_bill():
    try:
        ...
        if not account_number or not bill_identifier:
            return jsonify({"status": "failure", "message": "Account number and bill identifier are required."}), 400
        ...
        if not bill:
            conn.close()
            return jsonify({"status": "failure", "message": "Bill not found or incorrect account number."}), 404
        ...
        if status.lower() == "paid":
            conn.close()
            return jsonify({"status": "failure", "message": f"The bill with ID {bill_id} has already been paid."}), 400
        ...
        if not user_balance or user_balance[0] < amount:
            conn.close()
            return jsonify({"status": "failure", "message": "Insufficient balance to pay the bill."}), 400
        ...
        transaction_id = str(uuid.uuid4())[:8]  # Generating a simple 8 character transaction ID
        current_timestamp = datetime.now().strftime('%d/%m/%Y %H:%M:%S')  # Current timestamp in a readable format
        cursor.execute('''
...
def verify_bill():
    ...
    if not all([bill_id, main_category, sub_category]):
        return jsonify({"status": "failure", "message": "Bill ID, Main Category, and Sub Category are required."}), 400
    ...
    try:
        ...
        if bill:
            ...
            return jsonify({"status": "not_found", "message": "Bill not found in the database."})
...
def fetch_bill_details():
    ...
    account_number = request.args.get('account_number')
    bill_identifier = request.args.get('bill_identifier')  # This could be either the bill ID or the nickname
    ...
    try:
        ...
        if not bill_details:
            return jsonify({"status": "failure", "message": "No bill found with the given identifier."}), 404
