...
class ActionFetchMultipleDetails(Action):
    ...
    def run(
        ...
        account_number = tracker.latest_message.get("metadata", {}).get(
            "account_number"
        )

        if not account_number:
            dispatcher.utter_message(
                text="Account number not found. Please make sure you are logged in."
            )
            return []
        ...
        if entities:
            response = requests.get(
                f"http://127.0.0.1:5000/api/personal_info?account_number={account_number}&fields={','.join(entities)}"
            )
            if response.status_code == 200:
                ...
                result = "<br>".join(
                    [
                        f"<strong>{field.capitalize().replace('_', ' ')}:</strong> {value}"
                ...
                dispatcher.utter_message(
                    text=f"Here are the details: <br><br>{result}</br></br>"
                )
            else:
                dispatcher.utter_message(
                    text="Sorry, I couldn't fetch the details at this time."
                )
        else:
            dispatcher.utter_message(
                text="Please specify the details you want to know."
            )
...
class ActionGetBranchInfo(Action):
    ...
    def run(
        ...
        # Column mapping
        column_map = {"contact": 4, "timings": 7, "address": 3, "type": 6}

        # Case 1: No branch name or region is provided
        if not branch_name and not region:
            dispatcher.utter_message(
                text="Which branch or region are you asking about?"
            )
            return []
        ...
        if sat_open:
            ...
            if response.status_code == 200:
                ...
                if branches:
                    ...
                    dispatcher.utter_message(
                        text=f"No branches open on Saturday{' in ' + region if region else ''}."
                    )
            else:
                dispatcher.utter_message(
                    text="There was an error fetching the Saturday-open branch data."
                )
        ...
        if region:
            ...
            if response.status_code == 200:
                ...
                if branches:
                    dispatcher.utter_message(
                        text=f"Here are the branches in {region}:<br><br>"
                    )
                    for branch in branches:
                        dispatcher.utter_message(
                            text=f"<strong>{branch[2]}</strong><br><br> <strong>Address:</strong> {branch[3]} <br> <strong>Timings:</strong> {branch[7]}<br><br>"
                ...
                dispatcher.utter_message(
                    text="There was an error fetching the branch data."
                )
        ...
        if column and branch_name:
            ...
            if response.status_code == 200:
                ...
                if branch_info:
                    ...
                    dispatcher.utter_message(
                        text=f"Sorry, I don't have the information for {branch_name}."
                    )
            else:
                dispatcher.utter_message(
                    text="There was an error fetching the branch information."
                )
        ...
        if branch_name:
            ...
            if response.status_code == 200:
                ...
                if branch_info:
                    dispatcher.utter_message(
                        text=f"<br><br><strong>Branch Info:</strong><br><br>\n"
...
class ActionValidateAndAddPayee(Action):
    ...
    def run(
        ...
        if not payee_name or len(payee_name.strip()) <= 1 or payee_name.isdigit():
            dispatcher.utter_message(
                text="Payee name must contain more than one character and cannot be just spaces or numbers. Payee Addition Failed!"
            )
            return [
                SlotSet("payee_account_number", None),
                SlotSet("iban", None),
                SlotSet("bank_name", None),
                SlotSet("payee_name", None),
        ...
        if confirmation_pending:
            ...
            if confirmation == "affirm":
                ...
                response = requests.post(
                    "http://127.0.0.1:5000/api/add_payee", json=payload
                )
        ...
        response_check = requests.post(
            "http://127.0.0.1:5000/api/check_payee", json=payload
        )
...
class ActionFetchTransactions(Action):
    ...
    def run(
        ...
        for entity in entities:
            if entity["entity"] == "time":
                ...
                if time_info.get("type") == "value":
                    ...
                    if end_value:
                        try:
                            end_date = datetime.fromisoformat(end_value[:10]).strftime(
                                "%d/%m/%Y"
                            )
                        except ValueError:
                            dispatcher.utter_message(
        ...
        if response.status_code == 200:
            ...
            if transactions:
                dispatcher.utter_message(
                    json_message={"type": "transactions", "transactions": transactions}
                )
            else:
                dispatcher.utter_message(
                    json_message={
...
def _check_payee(
    from_account: str, to_account: str, dispatcher: CollectingDispatcher
) -> bool:
    endpoints = [
        (
            "http://127.0.0.1:5000/api/check_payee_info",
            {"account_number": from_account, "payee_account_number": to_account},
        )
    ...
    for url, payload in endpoints:
        try:
            ...
            if (
                ...
                and result.get("status", "not_found") == "found"
            ):
                acc_no = result.get("account_number")
                title = result.get("account_title")
                bank = result.get("bank_name")
                dispatcher.utter_message(
                    text=f"Account found: Account Number: {acc_no}, Account Title: {title}, Bank: {bank}. Proceeding..."
                )
                print(result)
                return True
...
def _generate_otp(from_account: str, dispatcher: CollectingDispatcher) -> str:
    try:
        response = requests.post(
            "http://127.0.0.1:5000/api/generate_otp",
            json={"account_number": from_account},
        )
        if response.status_code == 200:
...
def _validate_otp(
    from_account: str, otp: str, dispatcher: CollectingDispatcher
) -> bool:
    try:
        response = requests.post(
            "http://127.0.0.1:5000/api/validate_otp",
            json={"account_number": from_account, "otp": otp},
        )
...
def _execute_transfer(
    from_account: str, to_account: str, amount: float, dispatcher: CollectingDispatcher
) -> bool:
    try:
        response = requests.post(
        ...
        if response.status_code == 200 and response.json().get("status") == "success":
            dispatcher.utter_message(
                text=f"Transfer of {amount} to account {to_account} completed successfully."
            )
            return True
...
class ActionTransferFunds(Action):
    ...
    def run(
        ...
        # Retrieve slots and user message
        from_account = tracker.latest_message.get("metadata", {}).get("account_number")
        to_account = tracker.get_slot("to_account") or tracker.latest_message.get(
            "text"
        )
        transfer_amount = tracker.get_slot("transfer_amount")
        ...
        if transfer_amount:
            ...
            if amount_match:
                ...
                dispatcher.utter_message(
                    text="Invalid input. Please enter a valid amount for transfer."
                )
                return [
                    SlotSet("otp", None),
                    SlotSet("to_account", None),
        ...
        if int(transfer_amount) <= 0:
            dispatcher.utter_message(
                text=f"Wrong Input, Transfer failed. Please try again"
            )
            return [
                SlotSet("otp", None),
                SlotSet("to_account", None),
        ...
        if not waiting_otp:
            ...
            dispatcher.utter_message(
                text=f"OTP generated: {otp}. Please provide it to proceed."
            )
        ...
        if waiting_otp:
            ...
            # Perform the transfer
            if not _execute_transfer(
                from_account, to_account, transfer_amount, dispatcher
            ):
                print("Cant execute transfer")
                return [
                    SlotSet("otp", None),
...
class ActionShowPayees(Action):
    ...
    def run(
        ...
        if not account_number:
            dispatcher.utter_message(
                text="Account number is missing. Please log in to continue."
            )
            return []

        # API URL to fetch payees
        api_url = (
            f"http://127.0.0.1:5000/api/get_payees?account_number={account_number}"
        )
        ...
        try:
            ...
            if response.status_code == 200:
                try:
                    ...
                    if "payees" in response_data and isinstance(
                        response_data["payees"], list
                    ):
                        payees = response_data["payees"]

                        if not payees:
                            dispatcher.utter_message(
                                text="No payees found for your account."
                            )
                            return []
                        ...
                        for payee in payees:
                            message += (
                                f"<br><strong>Payee Name:</strong> {payee.get('payee_name', 'N/A')}<br>"
                        ...
                        dispatcher.utter_message(
                            text="Unexpected data format received from the server. Please try again later."
                        )

                except ValueError:
                    dispatcher.utter_message(
                        text="Failed to parse the response as JSON. Please try again later."
                    )

            else:
                dispatcher.utter_message(
                    text="Failed to retrieve payees. Please try again later."
                )

        except requests.RequestException as e:
            dispatcher.utter_message(
                text="An error occurred while retrieving payees. Please try again later."
            )
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if data["status"] == "success":
                    ...
                    if not complaints:
                        dispatcher.utter_message(
                            text="No complaints found for your account."
                        )
                        return []
                    ...
                    for complaint in complaints:
                        message += (
                            f"<br><strong>Complaint ID:</strong> {complaint['complaint_id']}<br>"
                    ...
                    dispatcher.utter_message(
                        text=data.get("message", "Failed to retrieve complaints.")
                    )
            else:
                dispatcher.utter_message(
                    text="Failed to retrieve complaints. Please try again later."
                )

        except requests.RequestException:
            dispatcher.utter_message(
                text="An error occurred while retrieving complaints. Please try again later."
            )
...
class ActionRegisterComplaint(Action):
    ...
    def run(
        ...
        response = requests.post(
            "http://127.0.0.1:5000/api/register_complaint", json=data
        )

        # Handle the response from the API
        if response.status_code == 200:
            dispatcher.utter_message(
                text=f"Your complaint has been registered successfully with Complaint ID: {complaint_id}."
            )
        else:
            dispatcher.utter_message(
                text="There was an issue registering your complaint. Please try again later."
            )
...
class ActionAddBill(Action):
    ...
    def run(
        ...
        if not all([account_number, main_category, sub_category, bill_id, nickname]):
            dispatcher.utter_message(
                text="Missing information. Please provide all the necessary bill details."
            )
            return []
        ...
        if response.status_code == 200:
            ...
            if bill_data["status"] == "found":
                ...
                return [
                    SlotSet("bill_confirmation_pending", True),
                    SlotSet("main_category", None),
                    SlotSet("sub_category", None),
                ]
            else:
                dispatcher.utter_message(
                    text="The specified bill does not exist. Please check the details and try again."
                )
                return [
                    SlotSet("main_category", None),
                    SlotSet("sub_category", None),
            ...
            dispatcher.utter_message(
                text="Unable to verify bill details at the moment. Please try again later."
            )
            return [
                SlotSet("main_category", None),
                SlotSet("sub_category", None),
...
class ActionValidateAndAddUserBill(Action):
    ...
    def run(
        ...
        if response.status_code == 200:
            dispatcher.utter_message(
                text=f"Bill with ID {bill_id} and nickname '{nickname}' has been successfully added to your account."
            )
        else:
            dispatcher.utter_message(
                text="There was an error adding the bill to your account. Please try again later."
            )
...
class ActionFetchBillDetails(Action):
    ...
    def run(
        ...
        if not bill_identifier:
            dispatcher.utter_message(
                text="Please provide a valid bill identifier (ID or nickname) to proceed."
            )
            return []
        ...
        try:
            ...
            if response.status_code == 200 and response_data.get("status") == "success":
                ...
                if status.lower() == "paid":
                    dispatcher.utter_message(
                        text=f"The bill '{bill_identifier}' has already been paid."
                    )
                    return []
                ...
                return [
                    SlotSet("bill_amount", amount),
                    SlotSet("waiting_for_bill_payment_confirmation", True),
                ]

            else:
                dispatcher.utter_message(
                    text=response_data.get(
                        "message", "Unable to fetch the bill details. Please try again."
                    )
                )

        except requests.RequestException as e:
            dispatcher.utter_message(
                text=f"An error occurred while communicating with the server: {e}"
            )
...
class ActionPayBill(Action):
    ...
    def run(
        ...
        if not bill_identifier:
            dispatcher.utter_message(
                text="Please provide a valid bill identifier (nickname or ID)."
            )
            return []

        if not bill_amount:
            dispatcher.utter_message(
                text="Unable to retrieve the bill amount. Please try again."
            )
            return []
        ...
        try:
            ...
            if response.status_code == 200 and response_data.get("status") == "success":
                ...
                dispatcher.utter_message(
                    text=response_data.get(
                        "message", "Unable to complete the payment. Please try again."
                    )
                )
